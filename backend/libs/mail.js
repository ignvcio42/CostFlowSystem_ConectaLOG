...
const transporter = nodemailer.createTransport({
  ...
  secure: false,
  auth: {
    user: "alphonso.paucek@ethereal.email",
    pass: "w2jxWPanRgg44jbaQN",
  },
});
...
export async function sendVerificationEmail(to, token) {
  ...
  const mailOptions = {
    from: '"Tu App" <leslie25@ethereal.email>', // Puedes dejarlo así para pruebas
    to,
...
export async function sendResetPasswordEmail(to, token) {
  ...
  const mailOptions = {
    from: '"Tu App" <leslie25@ethereal.email>',
    to,
...
export async function sendEmailToAdmins(adminEmails, user) {
  ...
  await transporter.sendMail({
    from: '"Sistema" <leslie25@ethereal.email>',
    to: adminEmails.join(","),
    subject,
    html,
  });
...
export async function sendUserAcceptedEmail(userEmail, nombre) {
  await transporter.sendMail({
    from: '"Plataforma" <leslie25@ethereal.email>',
    to: userEmail,
